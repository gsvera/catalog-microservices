...
public interface CatalogUserServiceRepository extends JpaRepository<CatalogUserService, Long> {
    @Query(value = "SELECT DISTINCT ON (ucs.id) ucs.id, ucs.name_service, ucs.min_price, ucs.max_price, ucsd.id, ucsd.file_base_64, COUNT(ucsd.id) OVER (PARTITION BY ucs.id) AS total_element \n" +
